...
class MiChroM:
    ...
    def addCapsuleConfinement(self, r_conf=5.0, z_conf=10.0, kr=30.0):
        R"""Adds a spherocylindrical (capsule) confinement potential.

        The potential is zero inside a cylinder of radius ``r_conf`` with
        hemispherical end caps beginning at ``|z| = z_conf``.  Outside the
        capsule it is harmonic:

        .. math::

            V = \frac{k_r}{2}\,\Theta(d_\mathrm{eff}-r_\mathrm{conf})
                (d_\mathrm{eff}-r_\mathrm{conf})^2,

        where

        .. math::

            d_\mathrm{eff} = \sqrt{r_{xy}^2 +
            [\Theta(|z|-z_\mathrm{conf})(|z|-z_\mathrm{conf})]^2}.

        Args:
            r_conf (float): Capsule radius in nanometers.
            z_conf (float): Half-length of the cylindrical section in
                nanometers.
            kr (float): Harmonic confinement force constant.

        This is the capsule potential used in the legacy HP simulations.
        Its global parameter names are retained so that time-dependent
        confinement schedules can update them through the OpenMM context.
        """
        capsule_energy = (
            "0.5 * kr * step(d_eff - r_conf) * (d_eff - r_conf)^2; "
            "d_eff = sqrt(r_xy^2 + (step(abs(z) - z_conf) * "
            "(abs(z) - z_conf))^2); "
            "r_xy = sqrt(x^2 + y^2)"
        )
        capsule_force = self.mm.CustomExternalForce(capsule_energy)
        capsule_force.addGlobalParameter("r_conf", r_conf)
        capsule_force.addGlobalParameter("kr", kr)
        capsule_force.addGlobalParameter("z_conf", z_conf)

        self.forceDict["CapsuleConfinement"] = capsule_force
        for i in range(self.N):
            capsule_force.addParticle(i, [])

    def addSphericalConfinement(self, r_conf=15.0, k_conf=30.0):
        R"""Adds a harmonic spherical confinement potential.

        The potential is zero inside a sphere of radius ``r_conf`` and is
        harmonic outside it:

        .. math::

            V(r) = \frac{k_\mathrm{sphere}}{2}\,
            \Theta(r-r_\mathrm{sphere})(r-r_\mathrm{sphere})^2.

        Args:
            r_conf (float): Sphere radius in OpenMiChroM length units.
            k_conf (float): Harmonic confinement force constant.

        This is the spherical potential used by the legacy HP sphere-collapse
        workflow.  Its global parameter names are retained so that the radius
        can be changed during an OpenMM simulation.
        """
        sphere_energy = (
            "step(r - r_sphere) * 0.5 * k_sphere * (r - r_sphere)^2; "
            "r = sqrt(x^2 + y^2 + z^2)"
        )
        sphere_force = self.mm.CustomExternalForce(sphere_energy)
        sphere_force.addGlobalParameter("r_sphere", r_conf)
        sphere_force.addGlobalParameter("k_sphere", k_conf)

        self.forceDict["SphericalConfinement"] = sphere_force
        for i in range(self.N):
            sphere_force.addParticle(i, [])
    ...
    def addExpansionTypes(self, mu=3.22, rc=1.78, initialA1=-2.68e-1):
        R"""Add the in-memory type-force ramp used in a expansion protocol.
        ...
        ``(lambda(t1, t2) + (A1 - initialA1) delta(t1) delta(t2)) * f(r)``.
        Here ``f(r)`` is the same tanh contact function used by
